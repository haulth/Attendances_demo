...
def get_students(request):
    ...
    try:
        ...
        if not students.exists():
            return JsonResponse({"message": "Không có học sinh nào được tìm thấy"}, status=404)
...
def save_attendance(request):
    try:
        ...
        logger.info(f"Nhận được dữ liệu: student_data={student_data}, date={date}, subject_id={subject_id}, session_id={session_id}")
...
def staff_update_attendance(request):
    try:
        ...
        subjects = Subject.objects.filter(id__in=teaching_schedules.values_list('class_name', flat=True).distinct())
        
        # láy tên trường dựa trên lịch dạy của gióa viên
        sessions = Course.objects.filter(id__in=teaching_schedules.values_list('school_name', flat=True).distinct())
    ...
    try:
        ...
        for student_dict in students:
            student = get_object_or_404(
                Student, student_id=student_dict.get('student_id'))  # Sửa từ admin_id sang student_id
    ...
    if request.method == "POST":
        ...
        for session_id in session_ids:
            try:
                ...
                if not CompletedTeachingSchedule.objects.filter(schedule=session).exists():
...
def staff_view_profile(request):
    ...
    form = StaffEditForm(request.POST or None, request.FILES or None, instance=staff, user_type="staff")
